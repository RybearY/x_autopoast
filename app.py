...
현재 일본 트위터의 트렌드인'{selected_trend}'와 AI 또는 데이터와의 연관성을 담은 일본어 트위터 게시물을 1개 작성해줘.
트위터 스타일로 자연스럽게 써주고, 너무 딱딱하지 않게 해줘. '{selected_trend}'의 관심이 있는 사람들이 봤을 때 자연스러운 내용이어야 해.
...
{reference_tweets}
        ...
        if post_type == "이미지":
            dalle_prompt = f"일본의 '{selected_trend}'와 관련된 주제를 중심으로 한, 파스텔톤의 귀엽고 아기자기한 스타일의 일러스트. 배경은 주제와 자연스럽게 어우러짐. 고해상도, 텍스트 없음."
